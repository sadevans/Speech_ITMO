...
class Wav2Vec2Decoder:
    ...
    def greedy_decode(self, logits: torch.Tensor) -> str:
        ...
        return self._ids_to_text(filtered)
        ...
        for t in range(T):
            ...
            for prefix, (p_b, p_nb) in top_beams:
                ...
                for c in range(V):
                    ...
                    if c == blank:
                        key = prefix
                        old_b, old_nb = new_beams.get(key, (NEG_INF, NEG_INF))
                        new_beams[key] = (_log_add(old_b, p_total + lp), old_nb)
                    elif len(prefix) > 0 and c == prefix[-1]:
                        key = prefix + (c,)
                        old_b, old_nb = new_beams.get(key, (NEG_INF, NEG_INF))
                        new_beams[key] = (old_b, _log_add(old_nb, p_b + lp))
                        key = prefix
                        old_b, old_nb = new_beams.get(key, (NEG_INF, NEG_INF))
                        new_beams[key] = (old_b, _log_add(old_nb, p_nb + lp))
                    else:
                        key = prefix + (c,)
                        old_b, old_nb = new_beams.get(key, (NEG_INF, NEG_INF))
                        new_beams[key] = (old_b, _log_add(old_nb, p_total + lp))
            beams = new_beams
        results = [
    ...
    def beam_search_with_lm(self, logits: torch.Tensor) -> str:
        ...
        def get_lm_score(prefix):
            ...
            n_words = len(words)
            if n_words > 0:
                s = self.lm_model.score(' '.join(words), bos=True, eos=False) * LOG10_TO_LN
            else:
                s = 0.0
            lm_cache[prefix] = (s, n_words)
            return s, n_words

        def combined_score(prefix, p_b, p_nb):
            acoustic = _log_add(p_b, p_nb)
            lm_s, n_w = get_lm_score(prefix)
            return acoustic + alpha * lm_s + beta * n_w
        ...
        for prefix, (p_b, p_nb) in beams.items():
            ...
            n_words = len(words)
            if n_words > 0:
                lm_s = self.lm_model.score(' '.join(words), bos=True, eos=True) * LOG10_TO_LN
            else:
                lm_s = 0.0
            score = acoustic + alpha * lm_s + beta * n_words
            if score > best_score:
                best_score = score
                best_text = text
        return best_text
    ...
    def lm_rescore(self, beams: List[Tuple[List[int], float]]) -> str:
        ...
        for token_ids, acoustic_score in beams:
            ...
            n_words = len(words)
            if n_words > 0:
                lm_score = self.lm_model.score(' '.join(words), bos=True, eos=True) * LOG10_TO_LN
            else:
                lm_score = 0.0
            score = acoustic_score + alpha * lm_score + beta * n_words
            if score > best_score:
                best_score = score
                best_text = text
        return best_text
